...
# * ADVERTISED_HOST_NAME: the external ip for the container
...
if [ ! -z "$ADVERTISED_HOST_NAME" ]; then
    echo "advertised listeners: $ADVERTISED_HOST_NAME"
    if grep -q "^advertised.listeners" $KAFKA_HOME/config/server.properties; then
        sed -r -i "s/#(advertised.listeners)=PLAINTEXT://(.*):9092/\1=$ADVERTISED_HOST_NAME/g" $KAFKA_HOME/config/server.properties
    else
        echo "advertised.listeners=PLAINTEXT://$ADVERTISED_HOST_NAME:9092" >> $KAFKA_HOME/config/server.properties
    fi
fi
