...
func testNewDataFlow() *DataFlow {
	return NewDataFlow(newTestHttpClient())
}
...
func TestDataFlowDegrade(t *testing.T) {
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Add(protocol.HeaderKeyDowngrade, protocol.HeaderValueDowngradeStatic)
	}))
	defer s.Close()
	...
	assert.Nil(t, err)
	assert.Equal(t, protocol.HeaderValueDowngradeStatic, header.Get(protocol.HeaderKeyDowngrade))
	assert.Equal(t, 10, a)
}
...
func TestDataFlowCircuitBreaker(t *testing.T) {
	...
	sentinelOptions := sentinel.Options{
	...
	// 第二次触发熔断
	var blockError *sentinel.BlockError
	statusCode, err = testNewDataFlow().newMethod(fasthttp.MethodGet, s.URL).CircuitBreaker("count", cbFunc).Do(context.Background())
	assert.NotNil(t, err)
