...
func TestShardedSentinelClient(t *testing.T) {
	// 有外部依赖，先忽略
	t.Skip()
	ctx := context.Background()
	client := NewShardedSentinelClient(&Options{
		Name:         "sharedsentinel01",
		ConnType:     "sentinel",
		MasterNames:  []string{"recncr6510", "recncr6511", "recncr6512"},
...
func BenchmarkShardedSentinelClient_Set(b *testing.B) {
	// 有外部依赖，先忽略
	b.Skip()
	ctx := context.Background()
	client := NewShardedSentinelClient(&Options{
